...
void function _CustomTDM_Init() {
	...
	//Callbacks
	AddCallback_OnClientConnected(void function (entity player) { thread OnClientConnected(player) })
	AddCallback_OnPlayerKilled(void function (entity victim, entity attacker, var damageInfo) { thread OnPlayerKilled(victim, attacker, damageInfo) })
	AddCallback_EntitiesDidLoad(__OnEntitiesDidLoad)
    AddSpawnCallback("prop_survival", void function (entity prop) { thread DissolveItem(prop) })
    ...
    if (FlowState_AllChat() && !FlowState_GAMEMODE() != FlowStateGamemodes.SURF) {
	...
	switch (FlowState_GAMEMODE())
	{
		case FlowStateGamemodes.TDMFFA:
		case FlowStateGamemodes.FIESTA:
		case FlowStateGamemodes.GUNGAME:
		...
		case FlowStateGamemodes.SURF:
				...
				thread RunSURF()
			break;
		case FlowStateGamemodes.PROPHUNT:
...
void function DissolveItem(entity prop)
{
    ...
    entity parrent = prop.GetParent()

    if (IsValid(parrent) && IsValid(prop) && parrent.GetClassName() == "prop_physics")
...
void function _OnPlayerConnected(entity player) 
{
    ...
    switch (GetGameState())
	{
        case eGameState.MapVoting:
            	...
            	if (FlowState_GAMEMODE() == FlowStateGamemodes.FIESTA)
        ...
        case eGameState.Playing:
            	...
            	//array <string> InValidMaps = [ "mp_rr_canyonlands_staging", "Skill trainer By Colombia", "Custom map by Biscutz", "White Forest By Zer0Bytes", "Brightwater By Zer0bytes" ]
            	...
            	if (DropPodOnSpawn && GetMapName() != "mp_rr_canyonlands_staging") {
                	_HandleRespawn(player)
...
vector function GetClosestEnemyToOrigin(vector origin, int ourTeam)
{
    ...
    foreach(player in GetPlayerArray_Alive()) {
        ...
        if (player.GetTeam() == ourTeam)
			continue

        float dist = Distance(player.GetOrigin(), origin)
        if (dist < enemyDist || enemyDist < 0) {
        	enemyDist = dist
...
void function __HighPingCheck(entity player)
{
	...
	else if (FlowState_GAMEMODE() == FlowStateGamemodes.TDMFFA)
	{
...
{
    if (FlowState_GAMEMODE() == FlowStateGamemodes.FIESTA && FlowState_FIESTADeathboxes())
    ...
    switch (GetGameState())
	{
		case eGameState.Playing:
				...
				void functionref() victimHandleFunc = void function (): (victim, attacker, damageInfo) {
					...
					int deathscore = victim.GetPlayerNetInt("assists")
					deathscore++
					victim.SetPlayerNetInt("assists", deathscore)

					if (FlowState_GAMEMODE() == FlowStateGamemodes.FIESTA)
						UpgradeShields(victim, true)

					if (FlowState_GAMEMODE() == FlowStateGamemodes.GUNGAME)
				...
				void functionref() attackerHandleFunc = void function (): (victim, attacker, damageInfo) {
					...
					if (IsValid(attacker) && attacker.IsPlayer() && IsAlive(attacker) && attacker != victim) {
						//Heal
						if (FlowState_GAMEMODE() == FlowStateGamemodes.FIESTA && FlowState_FIESTAShieldsStreak()) {
...
void function CreateFlowStateDeathBoxForPlayer(entity victim, entity attacker,
    var damageInfo) {
...
bool function ClientCommand_SpectateEnemies(entity player, array < string > args) {
    ...
    if (IsValid(player) && enemiesArray.len() > 0) {
        ...
        if (player.GetPlayerNetInt("spectatorTargetCount") > 0) {
            ...
            player.StopObserverMode()
            if (IsValidPlayer(player))
                player.TakeDamage(player.GetMaxHealth() + 1, null, null, {
                    damageSourceId = damagedef_suicide,
                    scriptType = DF_BYPASS_SHIELD
                })
            printf("Respawned!")
        } else {
            player.MakeInvisible()
